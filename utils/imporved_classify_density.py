...
Density thresholds:
  - empty:  P == 0%   (no vehicles detected)
  - light:  0 < P < 8%
  - medium: 8% ≤ P < 25%
  - high:   25% ≤ P < 50%
  - full:   P ≥ 50%

Note: Only vehicle class IDs (1-13) are counted; pedestrians (14) are excluded.
...
def parse_yolo_labels(
    label_path: Path, vehicle_classes: set[int] | None = None
) -> list[tuple[float, float, float, float]]:
    """
    Parse YOLO label file into list of (cx, cy, w, h) normalised coords.
    Returns empty list for empty / missing files.

    If *vehicle_classes* is given, only bounding boxes whose class ID
    belongs to the set are returned (filters out pedestrians, etc.).
    """
    ...
    for line in label_path.read_text().strip().splitlines():
        ...
        if len(parts) >= 5:
            cls_id = int(parts[0])
            if vehicle_classes is not None and cls_id not in vehicle_classes:
                continue
            cx, cy, w, h = (
                float(parts[1]),
                float(parts[2]),
...
# Main pipeline
# ──────────────────────────────────────────────────────────────────────
DENSITY_CLASSES = ("empty", "light", "medium", "high", "full")
...
def classify_density(
    ...
    for loc_id, loc_dir in locations:
        ...
        for img_path in image_files:
            ...
            boxes = parse_yolo_labels(label_path, vehicle_classes=VEHICLE_CLASSES)
        ...
        logger.info(
            "%s: %d images — empty=%d  light=%d  medium=%d  high=%d  full=%d",
            loc_name,
            total_loc,
            loc_counts["empty"],
            loc_counts["light"],
...
def main() -> None:
    ...
    parser.add_argument(
        "--histogram",
        action="store_true",
        help="Print density distribution histogram and export CSV (no copy).",
    )
